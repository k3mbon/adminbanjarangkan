...
import { collection, addDoc, getDocs, setDoc, onSnapshot } from 'firebase/firestore';
...
const GalleryForm = () => {
  ...
  useEffect(() => {
    const fetchImageUrls = async () => {
      try {
        ...
        const unsubscribe = onSnapshot(collection(db, 'album_foto'), (snapshot) => {
  ...
  const handleImageUpload = async () => {
    ...
    try {
      const newImageUrls = [];
      const albumRef = await addDoc(collection(db, 'album_foto'), { albumName });
      ...
      <div className="image-container">
        ...
        {imagePreviews.map((preview, index) => (
          <div key={index} className="image-item">
            <img src={preview} alt={`uploaded-img-${index}`} className="thumbnail" />
